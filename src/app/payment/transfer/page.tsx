...
import SendProofButton from "@/components/SendProofButton";
...
export default async function PaymentTransferPage({
  ...
  const adminEmail = process.env.ADMIN_PROOF_EMAIL || process.env.ADMIN_EMAILS?.split(",")[0]?.trim() || "";

  const totalFormatted = order ? formatArs(order.totalArs) : "";
    ...
    <main className="flex-1 flex flex-col">
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="max-w-sm w-full space-y-6">
          ...
          {/* Send proof buttons */}
          {order && (adminWhatsApp || adminEmail) && (
            <SendProofButton
              orderId={order.id}
              buyerName={order.buyerName}
              totalFormatted={totalFormatted}
              showTitle={order.show.title}
              adminWhatsApp={adminWhatsApp || undefined}
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 text-sm space-y-2">
            <p>
              Una vez que realices la transferencia, te vamos a enviar las
              entradas por email a{" "}
